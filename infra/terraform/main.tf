...
resource "aws_security_group" "efs" {
  ...
  description = "Allow API tasks to mount EFS."
  vpc_id      = aws_vpc.main.id
...
resource "aws_efs_file_system" "contacts" {
  creation_token   = "${local.name_prefix}-contacts"
  encrypted        = true
...
resource "aws_efs_mount_target" "contacts" {
  count = length(aws_subnet.private)

  file_system_id  = aws_efs_file_system.contacts.id
  subnet_id       = aws_subnet.private[count.index].id
  security_groups = [aws_security_group.efs.id]
}

resource "aws_efs_access_point" "contacts" {
  file_system_id = aws_efs_file_system.contacts.id
  ...
  volume {
    ...
    efs_volume_configuration {
      ...
      authorization_config {
        access_point_id = aws_efs_access_point.contacts.id
        iam             = "DISABLED"
      }
    }
  }
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        {
          name  = "CONTACTS_API_PORT"
          value = "7070"
        },
        {
          name  = "CONTACTS_CSV_PATH"
          value = "/data/contacts.csv"
        }
      ]
      mountPoints = [
        {
          sourceVolume  = "contacts-data"
          containerPath = "/data"
          readOnly      = false
        }
      ]
